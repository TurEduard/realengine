...
class db{
	...
	function connect($db_user, $db_pass, $db_name, $db_location = 'localhost', $show_error=1) {
		...
		} else $this->db_id = @mysqli_connect($db_location[0], $db_user, $db_pass, $db_name);

		if(!$this->db_id) {
			if($show_error == 1) $this->display_error(mysqli_connect_error(), '1');
			else {
		...
		if( version_compare($this->mysql_version, '5.5.3', '<') ) {
			die ("Engine required MySQL version 5.5.3 or greater. You need upgrade MySQL version on your server.");
		}
	...
	function select($query, $multi = false) {
		if(!$multi) {
			$this->query($query);
			$data = $this->get_row();
			$this->free();
